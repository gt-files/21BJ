...
def build_numeric_shoe_array(shoe_counts):
    ...
    numeric_shoe = np.array(cards_list, dtype=int)
    if len(numeric_shoe) == 0:
        raise ValueError("Shoe is empty, cannot build array!")
    return numeric_shoe
...
def simulate_dealer_hand_vectorized(dealer_card_val, shoe_counts, num_simulations):
    """Simulates dealer hands vectorized with proper ace handling."""
    shoe_array = build_numeric_shoe_array(shoe_counts)
    dealer_hands = np.full(num_simulations, dealer_card_val)
    dealer_totals = np.full(num_simulations, dealer_card_val)

    # Track soft ace counts
    soft_aces = np.where(dealer_hands == 11, 1, 0)
    ...
    while True:
        ...
        dealer_totals += np.where(hits, new_cards, 0)

        # Adjust for soft aces
        soft_aces += np.where((hits) & (new_cards == 11), 1, 0)
        over_bust = (dealer_totals > BLACKJACK) & (soft_aces > 0)
        dealer_totals = np.where(over_bust, dealer_totals - 10, dealer_totals)
        soft_aces = np.where(over_bust, soft_aces - 1, soft_aces)

    return dealer_totals

def monte_carlo_ev(player_cards, dealer_card_val, shoe_counts, action, simulations=SIMULATIONS):
    """Estimates the EV for a given action using Monte Carlo simulations."""
    ...
    if action in {"Stand", "Hit", "Double Down"}:
        multiplier = 2 if action == "Double Down" else 1
        ...
        for i in range(10):
            ...
            if action == "Hit":
                ...
                # Adjust EV based on outcomes
                win_conditions = ~bust_conditions & (
                    (dealer_totals > BLACKJACK) | (new_totals > dealer_totals)
                )
                lose_conditions = ~bust_conditions & (new_totals < dealer_totals)
                pushes += np.sum(~bust_conditions & (new_totals == dealer_totals))
                ev += np.sum(multiplier * win_conditions) - np.sum(multiplier * lose_conditions) - np.sum(multiplier * bust_conditions)
            else:
                # Handle Stand/Double Down
                win_conditions = (dealer_totals > BLACKJACK) | (player_total > dealer_totals)
                lose_conditions = player_total < dealer_totals
                pushes += np.sum(player_total == dealer_totals)
                ev += np.sum(multiplier * win_conditions) - np.sum(multiplier * lose_conditions)

            checkpoint_means.append(ev / ((i + 1) * chunk_size))

    elif action == "Split":
        # Split logic (recurse for two new hands)
        split_ev = 0
        split_card_val = player_cards[0]
        for _ in range(2):
            split_hand = [split_card_val]
            split_ev += monte_carlo_ev(split_hand, dealer_card_val, shoe_counts, "Hit", simulations // 2)[0]
        ev = split_ev / 2
